...
аномальные точки в операторские события, а карточка выбранной тревоги показывает
причины, значения признаков, вклад ML и рекомендацию оператору. Это учебный прототип, а не
...
## Защита проекта за 5–10 минут

- [Презентация PowerPoint](presentation/temperature-anomaly-monitor-defense.pptx) — 8 слайдов с задачей, архитектурой, методами, метриками и ограничениями.
- [Текстовая структура презентации](docs/PRESENTATION.md) — проверяемый источник тезисов и чисел.
- [Демонстрационный сценарий](docs/DEMO_SCRIPT.md) — точный маршрут по интерфейсу с таймингом и резервным вариантом.

Для показа используется встроенный демо-режим: он уже содержит данные, график,
тревоги и журнал событий, поэтому перед защитой не нужно вручную исправлять CSV.
...
| `presentation/` | Готовая PowerPoint-презентация для защиты. |
| `docs/DEMO_SCRIPT.md` | Репетиционный сценарий демонстрации на 5–10 минут. |
